...
|---|---|---|
| Bifrost `GET /api/v1/metrics` | `bifrost_clusters_total{state}`, `bifrost_clusters_by_project`, `bifrost_pool_nominal`, `bifrost_pool_resource_usage` | a `ServiceMonitor` with a bearer token from a local `viewer` user — the endpoint is Read on the cluster target |
