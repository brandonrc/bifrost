...
| `scrape-bifrost-and-ray.yaml` | the `ServiceMonitor` for `GET /api/v1/metrics` (bearer token of a local `viewer` user) and the `PodMonitor` for every Ray head and worker, relabelling `bifrost.dev/cluster-id` and `bifrost.dev/owner` onto every series. |
