...
| 7 | Group admins control profiles, images, CPU/mem/GPU, max workers | CRITICAL | **Partial** — `PolicyConfig` carries Prices/Quotas/Budgets/GPUDefaultSharing (CPU/mem/GPU); `serve --allowed-images` / `--max-workers` (2026-09-02, `api.Admission`) give the administrator an image-prefix allowlist and a per-cluster worker cap, enforced at create (400 + audit deny) and asserted by `r07_admin_controls`. **Profiles are not built** (no profile type), and the controls are deployment-level (platform admin), not per-group. The Wave 1 exit below claims req 7 *closed*; this row is the accurate one. |
