...
| 12 | Private storage (e.g. S3) from the cluster | CRITICAL | **Built** (2026-09-03, `private-storage`): administrators catalog Kubernetes Secrets as named `storage` entries in the policy row (`PUT /api/v1/settings/policy`, section-replace, validated as a unit: unique RFC 1123 names, RFC 1123 `secret_name`, `mount_path` required for `file` and never `/`, `/tmp`, `/home/ray` or under them, per-entry `projects` scoping); a cluster or service spec lists names, resolved once at admission (400 for an unknown name or one not open to the project; audit `storage_rejected`) and persisted as `StorageResolved` so a later catalog edit is never retroactive. The provisioner projects `env` entries as `envFrom.secretRef` and `file` entries as a read-only Secret volume on every pod template (RayCluster and RayService); the owned fingerprint covers the projection, so a stripped mount is drift. **Credentials never transit Bifrost:** the API carries names only, the live client's existence check fetches Secret *metadata* (`PartialObjectMetadata`, RBAC `secrets: get`) and fails the apply with a readable condition instead of `CreateContainerConfigError`, and `r12_private_storage` proves the random Secret value appears in no response body, no audit row and no pod manifest. `submit_job` storage resolution is a follow-up on package B's handler. |
