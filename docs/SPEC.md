...
| 12 | Private storage (e.g. S3) from the cluster | CRITICAL | **Built** (2026-09-03, `private-storage`): administrators catalog storage sources as named `storage` entries in the policy row (`PUT /api/v1/settings/policy`, section-replace, validated as a unit: unique RFC 1123 names, a known `source` — `secret` (default), `persistent_volume_claim` or `host_path` — with the source's own required field and no other source's, `mount_path` required for `file` and never `/`, `/tmp`, `/home/ray` or under them, per-entry `projects` scoping); a cluster or service spec lists names, resolved once at admission (400 for an unknown name or one not open to the project; audit `storage_rejected`) and persisted as `StorageResolved` so a later catalog edit is never retroactive. The provisioner projects `env` entries as `envFrom.secretRef` (secret source only) and `file` entries as a volume on every pod template (RayCluster and RayService) backed by the entry's Secret (read-only) or PersistentVolumeClaim/hostPath (read-write — data volumes, matching the pre-catalog deployments); the owned fingerprint covers the projection, so a stripped or re-pointed mount is drift. **Credentials never transit Bifrost:** the API carries names and paths only, the live client's existence check fetches Secret/claim *metadata* (`PartialObjectMetadata`, RBAC `secrets: get` / `persistentvolumeclaims: get`; a host path names no API-server object and is not checked) and fails the apply with a readable condition instead of `CreateContainerConfigError`/Pending, and `r12_private_storage` proves the random Secret value appears in no response body, no audit row and no pod manifest. Note: the workload namespace's PSS posture decides which sources can run — `enforce: baseline` admits claims but rejects hostPath pod volumes. `submit_job` storage resolution is a follow-up on package B's handler. |
