...
| 14 | Usage visibility: who requested what, duration, estimated cost | LOW | **Partial → producer built 2026-09-02**: the reconciler's metering pass (`controller.Meter`, `--metering-interval`, default 1 min) records each running cluster's held demand per resource, attributed to project and pool (`observed_spec`), with a closing zero when it stops; `GET /api/v1/usage` now reports resource-hours and, with a price sheet, cost (`r14_usage`). Attribution is per project/pool; per-user ("who") attribution and the job-history side-write remain open. Verified empty on grace 2026-09-02 before the change. |
