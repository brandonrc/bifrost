...
|---|---|---|---|
| 1 | Layout | Single module `github.com/bifrost-compute/bifrost`, `cmd/bifrost` + `internal/`, no `pkg/`. Authority: go.dev "Organizing a Go module"; Argo CD/Grafana/Temporal pattern. Split an `apis/` module only if external consumers ever need our types. | Multi-module repo (versioning pain, zero benefit today) |
...
| 10 | Module path | `github.com/bifrost-compute/bifrost` verbatim. Vanity domains are over-engineering; `internal/` keeps export surface near zero. | vanity import path |
