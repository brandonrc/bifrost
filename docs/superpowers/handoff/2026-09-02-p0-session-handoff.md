...
- Ruling: Task 10 omits the bifrost-pack checkout step — github.com/bifrost-compute/bifrost-pack does not exist (the pack repo has no remote; org decision pending with the user). pack/ tests skip in CI with a recorded reason until it does; a comment in ci.yml names the condition. Cost if wrong: pack template tests run only locally until the remote exists.
...
- `bifrost-pack`: **cannot be pushed** (no remote). It lives only at `/Users/khan/openteams/bifrost-pack`. Decision needed: create `github.com/bifrost-compute/bifrost-pack` (or under `nebari-dev`) and push `main` + `fix/nginx-runtime-resolver`.
