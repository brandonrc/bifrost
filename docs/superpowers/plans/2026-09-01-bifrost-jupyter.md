...
- [x] **Step 2:** Tests: env vars land in `runtime_env.env_vars`; submit returns a job id; status/logs readable via the gateway. Commit.
  - **AMENDED (T7).** "via the gateway" is superseded (head service, as above). **Log tail was not built** — deferred as a follow-up; only submit and status shipped.
