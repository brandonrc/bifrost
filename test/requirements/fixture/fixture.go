...
// HeadCPU is the head's CPU request/limit (they are equal: Guaranteed QoS).
// REQ_HEAD_CPU overrides the default of 1. On the 4-vCPU kind runner a
// 1-CPU head runs GCS, dashboard, agent, raylet and the job supervisor in
// one cgroup, and KubeRay's short dashboard timeouts then fail its RayJob
// status checks and RayService readiness polls (run 33802820554).
func HeadCPU() string {
	if v := os.Getenv("REQ_HEAD_CPU"); v != "" {
		return v
	}
	return "1"
}
...
func ClusterBodyWithImage(id, project, image string, ttl *int64) client.CreateClusterJSONRequestBody {
	...
	raw := fmt.Sprintf(`{"id":%q,"spec":{"name":%q,"project":%q,"ray_version":"2.56.0","image":%q,
		"head_cpu":%q,"head_memory":%q,"ttl_seconds":%s,
		"worker_groups":[{"name":"w","cpu":"1","memory":"2Gi","gpu":null,"min_replicas":%[8]d,"max_replicas":%[8]d,"replicas":%[8]d}]}}`,
		id, id, project, image, HeadCPU(), HeadMemory(), ttlJSON, WorkerReplicas())
...
func ServiceBody(name, project string) client.DeployServiceJSONRequestBody {
	...
	spec := map[string]any{
		...
		"serve_config_v2": ServeConfigV2(), "head_cpu": HeadCPU(), "head_memory": HeadMemory(),
...
func SubmitJobBody(id, project, entrypoint string, ttl *int32) client.SubmitJobJSONRequestBody {
	...
	raw := fmt.Sprintf(`{"id":%q,"spec":{"project":%q,"entrypoint":%q,"image":%q,"head_cpu":%q,"head_memory":%q,"ttl_seconds_after_finished":%s}}`,
		id, project, entrypoint, RayImage(), HeadCPU(), HeadMemory(), ttlJSON)
